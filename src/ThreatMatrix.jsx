...
import { analyzeDrones } from "./threatData";
...
export default function ThreatMatrix(){
  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      {/* Filters */}
      <div style={{display:"flex",gap:12,padding:"10px 20px",flexWrap:"wrap",alignItems:"center",borderBottom:"1px solid rgba(0,255,120,0.08)"}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search platform or manufacturer..." style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(0,255,120,0.15)",borderRadius:4,padding:"5px 10px",color:"#e4ecf4",fontSize:11,fontFamily:"'IBM Plex Mono',monospace",width:240,outline:"none"}}/>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:10,fontFamily:"'IBM Plex Mono',monospace"}}>
            ...
            <tbody>
              {filtered.map((d,i)=>{
                ...
                return <tr key={i} onClick={()=>setSel(isSel?null:d)} style={{cursor:"pointer",background:isSel?"rgba(0,255,120,0.08)":i%2===0?"rgba(255,255,255,0.01)":"transparent",borderBottom:"1px solid rgba(255,255,255,0.03)"}}
        ...
        {/* Detail Panel */}
        {sel&&<div style={{width:320,borderLeft:"1px solid rgba(0,255,120,0.15)",padding:16,overflow:"auto",background:"rgba(0,10,5,0.5)",flexShrink:0}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,margin:"12px 0",fontSize:9}}>
          ...
          <div style={{fontFamily:"'Oxanium',sans-serif",fontSize:8,color:"#00b4ff",letterSpacing:2,margin:"16px 0 8px"}}>DETECTION SCORES</div>
          ...
          <div style={{marginTop:12,fontSize:8,color:"#405060",lineHeight:1.5}}>
            {sel.pi<30&&<p style={{color:"#ff6666",marginBottom:4}}>⚠ Protocol injection ineffective — {sel.proto} not in known signature database. Command injection RTH/land will not work.</p>}
            {sel.jm<40&&<p style={{color:"#ff9944",marginBottom:4}}>⚠ Jamming difficult — {sel.proto} uses robust FHSS{sel.cell?" + cellular backup channel":""}</p>}
            {sel.gs<50&&<p style={{color:"#ffcc00",marginBottom:4}}>⚠ GPS spoofing limited — RTK provides cm-accuracy cross-check against spoofed position</p>}
            {sel.ad<40&&<p style={{color:"#a070d0",marginBottom:4}}>⚠ Acoustic detection weak — {sel.w}g platform too quiet for reliable detection beyond 200m</p>}
            {sel.rad<50&&<p style={{color:"#00cc66",marginBottom:4}}>⚠ Radar RCS marginal — small cross-section at {sel.w}g may reduce EchoGuard detection range</p>}
            {sel.or<=5&&<p style={{color:"#00cc66"}}>✓ All detection and defeat mechanisms effective. This platform is well-mitigated by the SV-1 architecture.</p>}
          </div>
        </div>}
      </div>
    </div>
