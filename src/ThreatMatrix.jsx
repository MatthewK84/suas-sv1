  ...
  return(
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",position:"relative"}}>
      <div style={{borderBottom:"1px solid rgba(0,255,120,0.08)",padding:mobile?"10px 12px":"10px 20px"}}>
        ...
        <div style={{display:"flex",gap:8,marginTop:6,flexWrap:"wrap"}}>{["CRITICAL","HIGH","ELEVATED","MODERATE","LOW"].filter(t=>tiers[t]>0).map(t=><div key={t} style={{display:"flex",alignItems:"center",gap:3}}><div style={{width:7,height:7,borderRadius:2,background:TC[t]}}/><span style={{fontSize:9,color:TC[t],fontWeight:700,fontFamily:"'Oxanium',sans-serif"}}>{t}</span><span style={{fontSize:11,color:"#e4ecf4",fontWeight:700}}>{tiers[t]}</span></div>)}</div>
